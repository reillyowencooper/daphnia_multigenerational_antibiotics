...
c.total <- data.frame(sequence = "total",
           t(apply(zeros_df[,-1], 2, sum, na.rm = TRUE))) # gen totals
names(c.total) <- c("sequence",0:4)
total_zeros <- sum(c.total[-1])

r.total <- data.frame(total = c(apply(zeros_df[,-1], 1, sum, na.rm = TRUE), # sequence totals
                                total_zeros)) # total zeros over gen and sequence
# check total zeros
sum(r.total[-nrow(r.total),]) == total_zeros # they are the same

zeros_table <- zeros_df %>% ungroup() %>% add_row(c.total) %>% add_column(r.total)
