...
Individual pairwise comparisons of community composition in the final generation of each treatment
to the first of each treatment, then comparing the final of each treatment to each other.
```{r pairwise permanovas, warning=FALSE, error=FALSE}
# Comparisons are: C5:C1, A5:A1, R1->4:A1, R2->3:A2, R3->2:A3, R4->1:A4, A5:C5, R1->4:C5, R2->3:C5, R3->2:C5, R4->1:C5.

# Making subset phyloseq objects
c5c1 <- subset_samples(microbiome_correct_gen, treatment %in% c("C5", "C1"))
a5a1 <- subset_samples(microbiome_correct_gen, treatment %in% c("A5", "A1"))
a5c5 <- subset_samples(microbiome_correct_gen, treatment %in% c("A5", "C5"))
r14c5 <- subset_samples(microbiome_correct_gen, treatment %in% c("R1→4", "C5"))
r23c5 <- subset_samples(microbiome_correct_gen, treatment %in% c("R2→3", "C5"))
r32c5 <- subset_samples(microbiome_correct_gen, treatment %in% c("R3→2", "C5"))
r41c5 <- subset_samples(microbiome_correct_gen, treatment %in% c("R4→1", "C5"))

pairwise_list <- list(c5c1, a5a1, a5c5, r14c5, r23c5, r32c5, r41c5)
...
pairwise_output <- plyr::ldply(pairwise_res, data.frame) %>% 
  mutate(variables = rep(c("treatment", "Residuals", "Total"), times = 7)) %>%
  mutate(group1 = c(rep("C5", 3),
                        rep("A5", 3),
                        rep("A5", 3),
                        rep("R1→4", 3),
                        rep("R2→3", 3),
