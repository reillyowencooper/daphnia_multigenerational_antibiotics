...
# Statistics
final_size_aov <- aov(final_size ~ gen*treatment, data = life_history)
final_size_aov_summary <- anova_summary(final_size_aov)
final_size_tukey <- tukey_hsd(final_size_aov)

# Saving statistics
write.csv(final_size_aov_summary, here("statistics", "final_size_anova.csv"))
write.csv(final_size_tukey, here("statistics", "final_size_tukey.csv"))
...
size_by_generation_plot <- ggplot(life_history, aes(gen, final_size)) +
  ...
  xlab("Generation") +
  ylab("Final size (mm)") +
  annotate("text", x = 1.25, y = 2.4, label = "Anova, p<0.0001") +
  annotate("text", x = 2, y = 3.8, label = "*") +
  annotate("text", x = 3, y = 3.8, label = "ns") +
  annotate("text", x = 4, y = 3.8, label = "ns") +
  annotate("text", x = 5, y = 3.8, label = "****") +
...
size_by_treatment_plot <- ggplot(life_history, aes(figure_labels, final_size)) +
  ...
  annotate("text", x = 4, y = 2.4, label = "Anova, p=0.0003")
```

Cumulative reproduction.
```{r cumulative reproduction, warning=FALSE, error=FALSE}
# Statistics
cum_repro_aov <- aov(cumulative_reproduction ~ gen*treatment, data = life_history)
cum_repro_aov_summary <- anova_summary(cum_repro_aov)
cum_repro_tukey <- tukey_hsd(cum_repro_aov)

# Saving statistics
write.csv(cum_repro_aov_summary, here("statistics", "cumulative_reproduction_anova.csv"))
write.csv(cum_repro_tukey, here("statistics", "cumulative_reproduction_tukey.csv"))
...
cum_repro_by_generation_plot <- ggplot(life_history, aes(gen, cumulative_reproduction)) +
  ...
  annotate("text", x = 1.25, y = 28, label = "Anova, p<0.0001") +
  annotate("text", x = 2, y = 32, label = "***") +
  annotate("text", x = 3, y = 32, label = "ns") +
  annotate("text", x = 4, y = 32, label = "ns") +
  annotate("text", x = 5, y = 32, label = "****") +
  scale_x_discrete(labels = gen_labels)
...
cum_repro_by_treatment_plot <- ggplot(life_history, aes(figure_labels, cumulative_reproduction)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        axis.title.x = element_blank()) +
  annotate("text", x = 4, y = 28, label = "Anova, p=0.31")
```
...
# REC v CTRL same gen
r11c1 <- subset_samples(microbiome_correct_gen, treatment %in% c("R1→1", "C1"))
r21c2 <- subset_samples(microbiome_correct_gen, treatment %in% c("R2→1", "C2"))
r31c3 <- subset_samples(microbiome_correct_gen, treatment %in% c("R3→1", "C3"))
r41c4 <- subset_samples(microbiome_correct_gen, treatment %in% c("R4→1", "C4"))
# REC v CTRL total gen
r11c2 <- subset_samples(microbiome_correct_gen, treatment %in% c("R1→1", "C2"))
r21c3 <- subset_samples(microbiome_correct_gen, treatment %in% c("R2→1", "C3"))
r31c4 <- subset_samples(microbiome_correct_gen, treatment %in% c("R3→1", "C4"))
r41c5 <- subset_samples(microbiome_correct_gen, treatment %in% c("R4→1", "C5"))
# REC v REC with same gen of recovery
...
r12r22 <- subset_samples(microbiome_correct_gen, treatment %in% c("R1→2", "R2→2"))
r12r32 <- subset_samples(microbiome_correct_gen, treatment %in% c("R1→2", "R3→2"))
r22r32 <- subset_samples(microbiome_correct_gen, treatment %in% c("R2→2", "R3→2"))
r13r23 <- subset_samples(microbiome_correct_gen, treatment %in% c("R1→3", "R2→3"))
# REC v CTRL 1 (excluding R1->1 - C1)
r21c1 <- subset_samples(microbiome_correct_gen, treatment %in% c("R2→1", "C1"))
r31c1 <- subset_samples(microbiome_correct_gen, treatment %in% c("R3→1", "C1"))
r41c1 <- subset_samples(microbiome_correct_gen, treatment %in% c("R4→1", "C1"))

pairwise_list <- list(c1a1, c2a2, c3a3, c4a4, c5a5, r11c1, r21c2, r31c3, r41c4,
                      r11c2, r21c3, r31c4, r41c5, r11r21, r11r31, r11r41, r21r31, 
                      r21r41, r31r41, r12r22, r12r32, r22r32, r13r23,
                      r21c1, r31c1, r41c1)
...
pairwise_output <- plyr::ldply(pairwise_res, data.frame) %>% 
  mutate(variables = rep(c("treatment", "Residuals", "Total"), times = 26)) %>%
